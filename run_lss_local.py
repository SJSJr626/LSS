...
For example::
  ...
  python emu_study_lvl1_ATM.py -s 1001
                               ...
                               -w /home/sjsuss626/scratch/madlab/emu/frstlvl/study
...
def subjectinfo(subject_id):
    ...
    for curr_run in [1]:
        ...
        # Variable to store all EVs for subsequent hit positive targets for current run
        data_pos = np.genfromtxt(base_proj_dir + 
                                         '/{0}/ses-S1/{0}_ses-S1_task-study_run-{1}_pos_events.tsv'.format(subject_id, curr_run),
                                         delimiter='\t', dtype=str)
        ...
        orig_all_pos_neg_neu_data = {'data_pos': data_pos,
                                          'data_neg': data_neg,
                                          'data_neu': data_neu}

        # ITERATE OVER THE KEYS OF THE DICTIONARY TO ISOLATE THE CONDITIONS OF INTEREST
        for curr_key in orig_all_pos_neg_neu_data.keys():
            ...
            copy_all_pos_neg_neu_data = dict(orig_all_pos_neg_neu_data)
            curr_condition_data = copy_all_pos_neg_neu_data.pop(curr_key)
            ...
            if curr_condition_data.shape[0] > 0:
                if curr_condition_data.size == 3: # ONLY ONE EVENT OF THIS CONDITION DURING THIS RUN
                    ...
                    for pending_key in copy_all_pos_neg_neu_data.keys():
                        pending_data = copy_all_pos_neg_neu_data[pending_key]
...
sids = ['sub-4125','sub-4024','sub-4090','sub-4097','sub-4107'] #FOR RUN 1 
...
datasource.inputs.field_template = dict(task_mri_files='%s/ses-S1/func/*_ses-S1_task-study_run-1_space-MNIPediatricAsym_cohort-5_res-2_desc-preproc_bold.nii.gz',
                                        motion_noise_files='%s/ses-S1/func/*_ses-S1_task-study_run-1_desc-confounds_timeseries.tsv') #add subject specific masks from fmri prep (dialate by 1)
...
frstlvl_lss_wf.run('MultiProc', plugin_args={'n_procs': 5})
